 ...
 * 设置页面附加属性
 */
public class PDFBulider extends PdfPageEventHelper {
     ...
     */
    public PDFBulider(String yeMei, int presentFontSize, Rectangle pageSize) {
        this.header = yeMei;
    ...
    public void addWatermark(PdfWriter writer) {
        ...
        Image image;
        try {
            image = Image.getInstance("a9.jpg");
...
//            image = Image.getInstance(PathUtil.getRootClassPath(PathCons.FILE_PDF_IMG));
            ...
            for (int k = 0; k < 5; k++) {
                for (int j = 0; j < 4; j++) {
                    image.setAbsolutePosition(500 * j, 470 * k);
